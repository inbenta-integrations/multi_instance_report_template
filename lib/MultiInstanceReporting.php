...
class MultiInstanceReporting
{
    ...
    private function setLanguageTags(string $dir)
    {
        if (!is_null($this->config["lang"]) &&  $this->config["lang"] !== "") {
            $file_lang = $dir.'/lang/'.$this->config["lang"].'.php';
            if (is_file($file_lang)) {
                $lang = include_once $file_lang;
                $this->lang_tags = $lang;
                return;
            }
        }
        $lang = include_once $dir.'/lang/en.php';;
        $this->lang_tags = $lang;
    }
    ...
    public function handleRequest(array $request)
    {
        ...
        switch ($action) {
            case 'language':
                echo json_encode(['lang' => $this->lang, 'tags' => $this->lang_tags]);
                break;
            case 'reports':
                $this->setApiClients();
                $params = [
                    ...
                    try {
                        ...
                        if (!is_null($client)) {
                            ...
                            if (isset($response[$instanceName]['sessions_total']->error)) {
                                $response[$instanceName]['sessions_total']->total_count = 0;
                            }
    ...
    private function processResponse(array $data, array $dates)
    {
        ...
        foreach ($data as $key => $instance) {
            $found = $instance["sessions_total"]->total_count - $instance["sessions_not_found"]->total_count;
            $response[$key] = [
                ...
                "sessions_no_clicks" => $found - $instance["sessions_clicks"]->total_count,
                "sessions_no_clicks_raw" => $found - $instance["sessions_clicks"]->total_count,
        ...
        foreach ($response as $key => $instance) {
            ...
            $response[$key]["sessions_total"] = "100%<br>(".number_format($sessions_total).")";

            $uqs_total = $instance["uqs_total"];
            $response[$key]["uqs_total"] = "100%<br>(".number_format($uqs_total).")";
            foreach ($instance as $column => $val) {
                if ($column !== "date" && strpos($column, "total") === false && strpos($column, "raw") === false) {
                    $total_to_process = strpos($column, "sessions") !== false ? $sessions_total : $uqs_total;
                    if ($total_to_process > 0) {
                        $response[$key][$column] = number_format(($val * 100) / $total_to_process, 2)."%<br>(".number_format($val).")";
                    }
                    else {
    ...
    private function downloadXLS(array $request)
    {
        if (isset($request["data"])) {
            $color_session = "style='background: #e1f0fd; text-align:center;'";
            $color_uqs = "style='background: #fbfbdc; text-align:center;'";
            $table = "<table width='100%' border='1'>";
            $table .= "<tr>";
            $table .= "<th>".$this->lang_tags["col_instance_name"]."</th>"; 
            $table .= "<th>".$this->lang_tags["col_date"]."</th>";
            $table .= "<th ".$color_session.">".$this->lang_tags["col_total_sessions"]."</th>";
            $table .= "<th ".$color_session.">".$this->lang_tags["col_found_sessions"]."</th>";
            $table .= "<th ".$color_session.">".$this->lang_tags["col_no_click_sessions"]."</th>";
            $table .= "<th ".$color_session.">".$this->lang_tags["col_not_found_sessions"]."</th>";
            $table .= "<th ".$color_uqs.">".$this->lang_tags["col_total_uqs"]."</th>";
            $table .= "<th ".$color_uqs.">".$this->lang_tags["col_found_uqs"]."</th>";
            $table .= "<th ".$color_uqs.">".$this->lang_tags["col_no_clicks_uqs"]."</th>";
            $table .= "<th ".$color_uqs.">".$this->lang_tags["col_not_found_uqs"]."</th>";
            $table .= "</tr>";
            foreach ($request["data"] as $key => $instance) {
                ...
                if ($key !== "totals") {
                    ...
                    $table .= "<td colspan='2' style='text-align:right; font-weight: bold;'>".$this->lang_tags["table_total"]." </td>";
                    $color_session = "style='font-weight: bold; text-align:center;'";
                    $color_uqs = "style='font-weight: bold; text-align:center;'";
                }
                $table .= "<td ".$color_session.">".str_replace("<br>", " ", $instance["sessions_total"])."</td>";
                $table .= "<td ".$color_session.">".str_replace("<br>", " ", $instance["sessions_found"])."</td>";
                $table .= "<td ".$color_session.">".str_replace("<br>", " ", $instance["sessions_no_clicks"])."</td>";
                $table .= "<td ".$color_session.">".str_replace("<br>", " ", $instance["sessions_not_found"])."</td>";
                $table .= "<td ".$color_uqs.">".str_replace("<br>", " ", $instance["uqs_total"])."</td>";
                $table .= "<td ".$color_uqs.">".str_replace("<br>", " ", $instance["uqs_found"])."</td>";
                $table .= "<td ".$color_uqs.">".str_replace("<br>", " ", $instance["uqs_without_click"])."</td>";
                $table .= "<td ".$color_uqs.">".str_replace("<br>", " ", $instance["uqs_without_answer"])."</td>";
                $table .= "</tr>";
            }
            $table .= "</table>";
    ...
    {
        if (isset($request["data"])) {
            $csv = $this->lang_tags["col_instance_name"].",";
            $csv .= $this->lang_tags["col_date"].",";
            $csv .= $this->lang_tags["col_total_sessions"].",";
            $csv .= $this->lang_tags["col_found_sessions"].",";
            $csv .= $this->lang_tags["col_no_click_sessions"].",";
            $csv .= $this->lang_tags["col_not_found_sessions"].",";
            $csv .= $this->lang_tags["col_total_uqs"].",";
            $csv .= $this->lang_tags["col_found_uqs"].",";
            $csv .= $this->lang_tags["col_no_clicks_uqs"].",";
            $csv .= $this->lang_tags["col_not_found_uqs"]."\n";
    ...
    {
        include_once(__DIR__ . '/../resources/index.php');
    }
}
